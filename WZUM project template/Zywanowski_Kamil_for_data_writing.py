...
def main():
    ...
    while current < stop:
        predicted_temperature = perform_processing(
            df_temperature_resampled.loc[(current + pd.DateOffset(minutes=15))],
    ...
    y_test = df_temperature_resampled['value'].to_numpy()
    y_predicted = df_temperature_resampled['predicted'].to_numpy()
